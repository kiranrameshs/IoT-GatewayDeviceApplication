...
public class CloudClientConnector implements ICloudClient {
	...
	/**
	 * Default. this.topicPrefix = this.topicPrefix.toLowerCase(); Depending on the
	 * cloud service, the topic names may or may not begin with a "/", so this code
	 * should be updated according to the cloud service provider's topic naming
	 * conventions
	 */
	...
	// public methods
	/**
	 * Connect the client and set the Data message listener if the client is not
	 * connected yet
	 */
	@Override
	...
	/**
	 * Set the Data message listener implemented, initialized on the device data manager
	 */
	@Override
	public boolean setDataMessageListener(IDataMessageListener listener) {
		_Logger.info("Initializing the data message listener");
		if (listener != null) {
	...
	/**
	 * Method to form the sensor data message and form all the args required by the
	 * pubmish message cpuData, memory Data etc from the GDA
	 */
	@Override
	...
	/**
	 * subscribe to the cloud topics to recieve the actuator command message from
	 * cloud if the event is triggered based on the sensor data
	 */
	@Override
	...
	/**
	 * Unsubscribe from the topic of the cloud broker using the create topic method
	 * and mqtt client connector
	 */
	@Override
	...
	// private methods
	/**
	 * Create a topic name using the device name and the resource type from the
	 * resoureEnum
	 * 
	 * @param resource
	 * @return
	...
	/**
	 * Publish the mesaage to cloud using cloud client parameters after connecting
	 * to the broker
	 * 
	 * @param resource
	 * @param itemName
