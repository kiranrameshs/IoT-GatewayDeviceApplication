...
public class DeviceDataManager implements IDataMessageListener {
	...
	private int tempSensorFloor;

	private int tempSensorCeiling;
	private boolean valueChangedFromPreviousIter;
	...
	public DeviceDataManager() {
		...
		this.tempSensorFloor = configUtil.getInteger(ConfigConst.GATEWAY_DEVICE, ConfigConst.TEMPERATURE_SENSOR_FLOOR);
		this.tempSensorCeiling = configUtil.getInteger(ConfigConst.GATEWAY_DEVICE,ConfigConst.TEMPERATURE_SENSOR_CEILING);
		initConnections();
	...
	/**
	 * handles incoming message by coverting message to Actuator is System state
	 * data
	 */
	@Override
	public boolean handleIncomingMessage(ResourceNameEnum resourceName, String msg) {
		_Logger.info("Handle Incoming Message");
	 ...
	 * check for a particular sensor type if the sensor data received from CDA is crossed 
	 * trigger the action through a actuator command by forming the actuator data
	 * convert the actuator data to JSON and pass it downstream for publishing to CDA
	 **/
	@Override
	public boolean handleSensorMessage(ResourceNameEnum resourceName, SensorData data) {
		...
		try {
			...
			if (data.getSensorType() == 3 && data.getValue() > this.tempSensorCeiling)
				_Logger.info("The value from temperature sensor is at normal");
				valueChangedFromPreviousIter = false;
			if (data.getSensorType() == 3 && data.getValue() < this.tempSensorCeiling) {
				_Logger.info("The value from temperature sensor has fallen below the floor value");
				...
				_Logger.info(" Actuator Command Message to CDA");
				_Logger.info(" Actuator Command Message is " + jsonData);
				if (valueChangedFromPreviousIter == false) {
					this.mqttClient.publishMessage(ResourceNameEnum.CDA_DISPLAY_RESPONSE_RESOURCE, jsonData, 1);
					valueChangedFromPreviousIter = true;
				}
	...
	/**
	 * starts data device manager. we check the boolean values of each protocol and
	 * decide whether to enable that connection
	 * 
	 */
	...
	/**
	 * stops data device manager. we check the boolean values of each protocol and
	 * decide whether to disable that connection
	 */
